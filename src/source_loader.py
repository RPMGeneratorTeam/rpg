...
class SourceLoader:
    ...
    def _get_compression_method(self, name):
        ...
        with open(name, 'rb') as f:
            header = f.read(max(len(mode) for mode in self._tar_compression_mode.keys()))
    ...
    def _create_archive(self, path, source_dir, compression=TAR_GZIP):
        ...
        mode = self._tar_compression_mode.get(self._tar_compression_type.get(compression))
    ...
    def load_sources(self, path, source_dir, compression=TAR_GZIP):
        ...
        if (os.path.isfile(path)):
            ...
            else: # first element is not a directory -> rootdir does not exist
                Spec.create_root_directory = True
            ...
            try:
                ...
                print("error: Extraction of '{}' failed: {}".format(path,
                                                              os.strerror(e.errno)),
                      file=sys.stderr)
                return -1
                    ...
                    try:
                        ...
                        if e.errno == errno.EPERM or e.errno == errno.EACCES:
                            print("error: failed creating directory tree at {}: {}"
                                  .format(source_dir, os.strerror(e.errno)),
                                  file=sys.stderr)
                        return -1
                else:
                    return -1
